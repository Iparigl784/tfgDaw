...
export default function EncuestaCreate() {
  ...
  const handleCreate = async (payload) => {
    ...
    setErrors({});
    try {
      const res = await apiRequest(endpoints.encuestas, { method: 'POST', body: payload });
      toast.success('Encuesta creada correctamente.');
      const nueva = res?.data ?? res;
      if (nueva?.id) navigate(`/encuestas/${nueva.id}`);
      else navigate('/encuestas');
    } catch (err) {
      if (err.status === 422) {
